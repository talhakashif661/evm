...
import { fetchMyBookings, cancelBooking, checkIn, createPaymentIntent } from '../store/slices/bookingSlice';
...
export default function Bookings() {
  ...
  useEffect(() => {
    ...
    const refresh = () => dispatch(fetchMyBookings({ page, limit: 10, ...(filter && { status: filter }) }));
    const handleStatusChanged = ({ status, reason }) => {
      if (status === 'CANCELLED' && reason === 'NO_SHOW') toast.warning('A booking was cancelled — check-in window expired');
      if (status === 'CANCELLED' && reason === 'PAYMENT_TIMEOUT') toast.warning('A booking was cancelled — payment window expired');
  ...
  const handleConfirmCheckIn = async (e) => {
    e.preventDefault();
    const res = await dispatch(checkIn({ id: checkInModal.id, evId: selectedEvId !== checkInModal.evId ? selectedEvId : undefined }));
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '44px 32px' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Your charging session history</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
        {statuses.map(s => (
          <button key={s}
            ...
            style={{
              padding: '6px 16px', borderRadius: 4, cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600,
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={<button className="btn-primary" onClick={() => dispatch(fetchMyBookings({ page, limit: 10, ...(filter && { status: filter }) }))}>Try Again</button>}
        />
      ) : bookings.length === 0 ? (
        <EmptyState
          icon={<Calendar size={48} color="var(--text-muted)" strokeWidth={1.5} />}
          title={filter ? 'No bookings match this filter' : 'No bookings found'}
          subtitle={filter ? 'Try a different status, or view all your bookings.' : 'Book a charging slot to get started.'}
          action={filter
            ? <button className="btn-outline" onClick={() => setFilter('')}>View All Bookings</button>
            : <Link to="/stations"><button className="btn-primary">Find a Station <ArrowRight size={14} /></button></Link>}
        />
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {bookings.map((b, i) => {
            const noShowDeadline = new Date(new Date(b.startTime).getTime() + NO_SHOW_MINUTES * 60 * 1000);
            ...
            const canCheckIn = b.status === 'CONFIRMED' && (!b.plannedEndTime || new Date() < noShowDeadline);
            return (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                    <h2 style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontWeight: 600, fontSize: '1.3rem', letterSpacing: '0.02em' }}>
                      {b.slot?.station?.name}
                    </h2>
                    <BookingStatusBadge status={b.status} />
                  </div>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginBottom: 4 }}>
                    Slot #{b.slot?.slotNumber} · {b.ev?.model} · {new Date(b.startTime).toLocaleString()}
                    {b.plannedEndTime && <> → {new Date(b.plannedEndTime).toLocaleString()}</>}
                  </p>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                    <MapPin size={13} /> {b.slot?.station?.address}, {b.slot?.station?.city}
                    {b.totalCost != null && <span style={{ color: 'var(--gold)', marginLeft: 12, fontWeight: 600 }}>{toPKR(b.totalCost)}</span>}
                    {b.overageAmount != null && <span style={{ color: 'var(--warning)', marginLeft: 8, fontWeight: 600 }}>+{toPKR(b.overageAmount)} overage</span>}
                  </p>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {canCheckIn && (
                    <button
                      ...
                      className="btn-gold"
                      style={{ padding: '6px 16px', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: 6 }}
                    >
                  ...
                  {b.status === 'CHECKED_IN' && (
                    <button
                      ...
                      style={{ padding: '6px 16px', fontSize: '0.8rem', display: 'inline-flex', alignItems: 'center', gap: 6, opacity: payingBookingId === b.id ? 0.6 : 1 }}
                    >
                      <CreditCard size={14} /> {payingBookingId === b.id ? 'Loading...' : `Pay ${b.totalCost != null ? toPKR(b.totalCost) : ''}`}
                    </button>
                  )}
      ...
      <Modal show={!!checkInModal} onClose={() => setCheckInModal(null)} title="Confirm Check-In">
        <form onSubmit={handleConfirmCheckIn}>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginBottom: 16 }}>
            Confirm you&apos;ve arrived at <strong style={{ color: 'var(--text-primary)' }}>{checkInModal?.slot?.station?.name}</strong>, Slot #{checkInModal?.slot?.slotNumber}.
          </p>
          <div className="mb-4">
            <label className="form-label" htmlFor="checkin-ev">Charging Vehicle</label>
            <select id="checkin-ev" className="form-control" value={selectedEvId} onChange={e => setSelectedEvId(e.target.value)} required>
              {evs.map(ev => <option key={ev.id} value={ev.id}>{ev.model}{ev.licensePlate ? ` (${ev.licensePlate})` : ''}</option>)}
            </select>
            <small style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Changed your mind about which EV? Pick a different one here.</small>
          </div>
          <button type="submit" className="btn-gold" style={{ width: '100%', padding: 12 }}>Confirm Check-In</button>
        </form>
      </Modal>
