...
const roles = [
  { value:'EV_USER',       label:'EV User',       desc:'Book charging slots & bid in auctions',
    icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a5 5 0 1 1 0 10A5 5 0 0 1 12 2zm0 12c5.33 0 8 2.67 8 4v2H4v-2c0-1.33 2.67-4 8-4z"/></svg> },
  { value:'STATION_OWNER', label:'Station Owner',  desc:'List & manage your charging station',
    icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M2 20V8l10-6 10 6v12H2zm10-2a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/></svg> },
];
...
export default function Register() {
  ...
  const errors = {
    ...
    email: touched.email && form.email && !EMAIL_RE.test(form.email) ? 'Enter a valid email address' : '',
    password: touched.password && form.password && form.password.length < 6 ? 'Must be at least 6 characters' : '',
    confirmPassword: touched.confirmPassword && confirmPassword && confirmPassword !== form.password ? "Passwords don't match" : '',
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    setTouched({ firstName: true, email: true, password: true, confirmPassword: true });
    if (!firstName.trim() || !EMAIL_RE.test(form.email) || form.password.length < 6 || confirmPassword !== form.password) {
      return;
    }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24,
      ...
      <SEO
        ...
        noIndex
      />
      <motion.div initial={{ opacity:0, y:24 }} animate={{ opacity:1, y:0 }} style={{ width:'100%', maxWidth:500 }}>
        ...
        <div style={{ textAlign:'center', marginBottom:32 }}>
          ...
          <h1 style={{ fontFamily:'Georgia, "Times New Roman", serif', fontSize:'2.4rem', fontWeight:700, letterSpacing:'0.02em', marginBottom:6 }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:28 }}>
            {roles.map(r => (
              <button key={r.value} type="button" onClick={() => setForm({...form, role:r.value})}
                style={{ flex:1, padding:'10px 6px', borderRadius:8, cursor:'pointer', textAlign:'center',
                ...
                <span style={{ fontSize:'0.78rem', fontWeight:700, fontFamily:'Inter' }}>{r.label}</span>
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-grid-2col" style={{ marginBottom:12 }}>
              <div>
                ...
                <input id="reg-firstName" type="text" className="form-control" placeholder="John"
                  value={firstName}
                  onChange={e => setFirstName(e.target.value)}
                  onBlur={() => markTouched('firstName')}
                  required />
                {errors.firstName && <small style={{ color: 'var(--error)' }}>{errors.firstName}</small>}
              </div>
              <div>
                <label className="form-label" htmlFor="reg-lastName">Last Name</label>
                <input id="reg-lastName" type="text" className="form-control" placeholder="Doe"
                  value={lastName}
            ...
            <div className="mb-3">
              ...
              <input id="reg-email" type="email" className="form-control" placeholder="you@example.com"
                ...
                onBlur={() => markTouched('email')} required />
            ...
            <div className="mb-3">
              ...
              <input id="reg-phone" type="tel" className="form-control" placeholder="+92 300 0000000"
            ...
            <div className="mb-3">
              ...
              <input id="reg-password" type="password" className="form-control" placeholder="Min. 6 characters"
                ...
                onBlur={() => markTouched('password')} required minLength={6} />
            ...
            <div className="mb-4">
              ...
              <input id="reg-confirmPassword" type="password" className="form-control" placeholder="Re-enter your password"
                value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                onBlur={() => markTouched('confirmPassword')} required />
