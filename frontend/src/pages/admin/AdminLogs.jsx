...
import { ScrollText, ShieldCheck, ShieldX, UserMinus, UserCog, Zap, KeyRound, Trash2, Star } from 'lucide-react';
...
function ActionBadge({ action }) {
  const meta = ACTION_META[action] || { label: action, color: 'var(--text-secondary)', icon: ScrollText };
  const Icon = meta.icon;
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 5,
      color: meta.color, fontWeight: 700, fontSize: '0.78rem',
      whiteSpace: 'nowrap'
    }}>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 className="section-title">Audit <span>Log</span></h1>
      ...
      {loading ? (
        <div className="ev-card" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
