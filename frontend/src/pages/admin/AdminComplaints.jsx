...
export default function AdminComplaints() {
  ...
  const handleDelete = (c) => {
    if (window.confirm(`Delete the complaint "${c.subject}" from ${c.name}? This cannot be undone.`)) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 className="section-title">User <span>Complaints</span></h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.88rem' }}>
          Complaints filed through the Contact Us page. Reply on WhatsApp or Gmail, then delete once handled.
        </p>
      </div>
            ...
            {complaints.map((c, i) => (
              <motion.div key={c.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 8 }}>
                  <h2 style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontWeight: 700, fontSize: '1.4rem', letterSpacing: '0.02em' }}>{c.subject}</h2>
                ...
                {/* Who */}
                <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 12, color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                    <User size={13} /> {c.name}{c.userId ? '' : ' (guest)'}
                  </span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                ...
                {/* Message */}
                <p style={{
                  whiteSpace: 'pre-wrap', color: 'var(--text-primary)', fontSize: '0.92rem',
                  background: 'var(--bg-elevated)', borderRadius: 6, padding: '14px 16px', marginBottom: 16
                }}>
                  {c.message}
                </p>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => handleDelete(c)} className="btn-danger-sm"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 16px', fontSize: '0.8rem', marginLeft: 'auto' }}>
