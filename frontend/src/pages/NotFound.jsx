...
export default function NotFound() {
  ...
  return (
    <div style={{
      ...
      <SEO title="Page Not Found" description="The page you're looking for doesn't exist or may have been moved. Return to ChargeEV to continue." noIndex />
      <ZapOff size={56} color="var(--primary)" strokeWidth={1.5} />
      <h1 style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontSize: '3rem', fontWeight: 700, letterSpacing: '0.02em', margin: '22px 0 10px' }}>
      ...
      <Link to={homePath}>
        <button className="btn-primary" style={{ padding: '12px 28px' }}>
          <Home size={16} /> Back to {!token ? 'Home' : user?.role === 'ADMIN' ? 'Admin Dashboard' : 'Dashboard'}
        </button>
      </Link>
    </div>
  );
