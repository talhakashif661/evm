...
router.put('/profile', authenticate, async (req, res, next) => {
  try {
    ...
    if (avatar !== undefined) {
      if (avatar === null || avatar === '') {
        ...
        if (!isValidImageDataUrl(avatar)) {
          return res.status(400).json({ success: false, message: 'Avatar must be a valid image (JPEG, PNG, or WebP)' });
        }
    ...
    const updated = await prisma.user.update({
      ...
      data,
      select: { id: true, name: true, email: true, role: true, phone: true, avatar: true, isVerified: true }
    });
