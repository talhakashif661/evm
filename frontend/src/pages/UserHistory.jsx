...
const statusBadge = (s) => {
  const map = { COMPLETED:'badge-success', CONFIRMED:'badge-primary', ACTIVE:'badge-warning', CANCELLED:'badge-danger' };
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      setStats({
        ...
        totalEnergy: completed.reduce((a,b) => a + (b.slot?.powerKw||0) * ((new Date(b.endTime)-new Date(b.startTime))/3600000||0), 0),
    ...
    <div className="page-container">
      ...
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
        <div style={{ marginBottom:28 }}>
          ...
          <p style={{ color:'var(--text-secondary)', fontSize:'0.9rem' }}>Your complete EV charging activity log</p>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:16, marginBottom:32 }}>
          {statsLoading ? (
            ...
            { label:'Energy Used', value:`${stats.totalEnergy.toFixed(1)} kWh`, color:'var(--accent)' },
            ...
            <div key={s.label} className="stat-card" style={{ textAlign:'center', padding:20 }}>
              <p style={{ fontSize:'1.8rem', fontWeight:700, fontFamily:'Inter', color:s.color }}>{s.value}</p>
              <p style={{ color:'var(--text-muted)', fontSize:'0.75rem', textTransform:'uppercase', letterSpacing:'0.06em', marginTop:4 }}>{s.label}</p>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="ev-card" style={{ padding: 48 }}>
            <EmptyState
              ...
              action={<Link to="/stations"><button className="btn-primary">Find a Station <ArrowRight size={14} /></button></Link>}
            />
            <div className="row g-4" style={{ marginTop: 32, textAlign: 'left' }}>
              <div className="col-12 col-md-4" style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <BatteryCharging size={20} color="var(--accent-gold-dark)" style={{ flexShrink: 0, marginTop: 2 }} />
                <div>
                  <strong style={{ fontSize: '0.9rem' }}>Energy tracking</strong>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', margin: 0 }}>See exactly how many kWh each session used.</p>
                </div>
              </div>
              <div className="col-12 col-md-4" style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <Receipt size={20} color="var(--accent-gold-dark)" style={{ flexShrink: 0, marginTop: 2 }} />
                <div>
                  <strong style={{ fontSize: '0.9rem' }}>Cost breakdown</strong>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', margin: 0 }}>Every session&apos;s price, at the rate you actually paid.</p>
                </div>
              </div>
              <div className="col-12 col-md-4" style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                <TrendingUp size={20} color="var(--accent-gold-dark)" style={{ flexShrink: 0, marginTop: 2 }} />
                <div>
                  <strong style={{ fontSize: '0.9rem' }}>Usage trends</strong>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', margin: 0 }}>Spot your charging patterns as history builds up.</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <table className="ev-table">
                ...
                <tbody>
                  {history.map((b, i) => {
                    ...
                    const energyKwh = durationMins && b.slot?.powerKw ? (durationMins/60 * b.slot.powerKw).toFixed(2) : '—';
                    return (
                      <motion.tr key={b.id} initial={{ opacity:0 }} animate={{ opacity:1, transition:{ delay: i*0.03 } }}>
                        <td style={{ color:'var(--text-muted)', fontSize:'0.8rem' }}>{(page-1)*15+i+1}</td>
                        <td>
                          <p style={{ fontWeight:600, fontSize:'0.88rem' }}>{b.slot?.station?.name || '—'}</p>
                          <p style={{ color:'var(--text-muted)', fontSize:'0.74rem' }}>{b.slot?.station?.address}</p>
                        </td>
                        <td style={{ color:'var(--text-secondary)', fontSize:'0.85rem' }}>{b.slot?.station?.city || '—'}</td>
                        ...
                        <td style={{ color:'var(--text-secondary)', fontSize:'0.82rem', whiteSpace:'nowrap' }}>
                          ...
                          <span style={{ color:'var(--text-muted)', fontSize:'0.75rem' }}>{start.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</span>
                        </td>
                        <td style={{ color:'var(--text-secondary)', fontSize:'0.82rem', whiteSpace:'nowrap' }}>
                          {end ? (<>{end.toLocaleDateString()}<br/><span style={{ color:'var(--text-muted)', fontSize:'0.75rem' }}>{end.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</span></>) : '—'}
                        </td>
                        <td style={{ color:'var(--text-secondary)', fontSize:'0.85rem' }}>
                          {durationMins ? `${durationMins} min` : '—'}
                        </td>
                        <td style={{ color:'var(--accent)', fontWeight:600 }}>{energyKwh} {energyKwh !== '—' ? 'kWh' : ''}</td>
