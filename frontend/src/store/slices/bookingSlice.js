...
export const fetchMyBookings = createAsyncThunk('bookings/fetchAll', async (params = {}, { rejectWithValue }) => {
  try {
...
export const checkIn = createAsyncThunk('bookings/checkIn', async ({ id, evId }, { rejectWithValue }) => {
  try {
...
export const createPaymentIntent = createAsyncThunk('bookings/createPaymentIntent', async (id, { rejectWithValue }) => {
  try {
...
export const fetchPayments = createAsyncThunk('bookings/fetchPayments', async (_, { rejectWithValue }) => {
  try {
...
const bookingSlice = createSlice({
  name: 'bookings',
  initialState: { bookings: [], pagination: null, loading: false, error: null, payments: [], paymentsLoading: false },
  reducers: {},
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createBooking.fulfilled, (state, action) => {
        ...
        toast.success(
          createElement('span', null,
            'Booking confirmed! ',
            createElement(Link, { to: '/bookings', style: { color: 'inherit', fontWeight: 700, textDecoration: 'underline' } }, 'View My Bookings')
          ),
          { autoClose: 6000 }
        );
      })
      ...
      .addCase(fetchPayments.pending, (state) => { state.paymentsLoading = true; })
