...
export default function StationReport() {
  ...
  const buildChartData = () => {
    ...
    completed.forEach((b) => {
      ...
      let key = '';
      if (period === 'daily')
        key = d.toLocaleDateString('en-PK', { day: '2-digit', month: 'short' });
      if (period === 'weekly') {
        const w = getWeek(d);
        key = `W${w} ${d.getFullYear()}`;
      }
      if (period === 'monthly')
        key = d.toLocaleDateString('en-PK', { month: 'short', year: 'numeric' });
        ...
        >
          {[
            ...
            <div key={s.label} className="stat-card" style={{ textAlign: 'center', padding: 20 }}>
              <p
                style={{ fontSize: '1.7rem', fontWeight: 700, fontFamily: 'Inter', color: s.color }}
              >
                {s.value}
              </p>
              <p
            ...
            <table className="ev-table">
              ...
              <tbody>
                {bookings.length === 0 ? (
                  ...
                  pagedBookings.map((b) => {
                    const hrs = b.endTime
                      ? (new Date(b.endTime) - new Date(b.startTime)) / 3600000
                      : 0;
                    ...
                    return (
                      <tr key={b.id}>
                        ...
                        <td>
                          <span className={map[b.status] || 'badge-info'}>{b.status}</span>
                        </td>
                      </tr>
                    );
                  })
