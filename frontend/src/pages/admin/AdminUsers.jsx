...
import { fetchUsers, toggleBlockUser as blockUser, deleteUser, promoteToAdmin } from '../../store/slices/adminUsersSlice';
import { SkeletonRow } from '../../components/Skeleton';
import SEO from '../../components/SEO';

const roleBadge = r => r==='ADMIN'?'badge-danger':r==='STATION_OWNER'?'badge-warning':'badge-primary';
...
export default function AdminUsers() {
  ...
  useEffect(() => { dispatch(fetchUsers({ page, limit: 20 })); }, [dispatch, page]);
          ...
          <div className="ev-card" style={{ overflow:'hidden' }}>
            <div style={{ overflowX:'auto' }}>
              <table className="ev-table">
                <thead>
                  <tr><th>#</th><th>Name</th><th>Email</th><th>Role</th><th>Verified</th><th>Blocked</th><th>Joined</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {(users||[]).map((u,i) => (
                    <tr key={u.id}>
                      ...
                      <td>
                        <span style={{ color: u.isVerified?'var(--success)':'var(--danger)', fontWeight:600, fontSize:'0.82rem' }}>
                          {u.isVerified ? 'Yes' : 'No'}
                        </span>
                      </td>
                      <td>
                        <span style={{ color: u.isBlocked?'var(--danger)':'var(--success)', fontWeight:600, fontSize:'0.82rem' }}>
                        ...
                        <div style={{ display:'flex', gap:6, flexWrap: 'wrap' }}>
                          {u.role !== 'ADMIN' && (
                            <button className="btn-success-sm"
                              onClick={() => { if (window.confirm(`Promote ${u.name} to Admin? This cannot be undone from here.`)) dispatch(promoteToAdmin(u.id)); }}>
                              Promote to Admin
                            </button>
                          )}
                          ...
                          <button className="btn-danger-sm"
                            onClick={() => {
                              const warning = u.role === 'STATION_OWNER'
